...
export default function EditProduct() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("Updating product document:", id, updatedProduct);
    const ok = await DbService.updateProduct(id, updatedProduct);
    if (ok) {
      alert(`Product ${id} updated successfully!`);
      refreshCatalog();
      router.push("/admin/products");
    } else {
      alert("Failed to save product changes to Firestore.");
    }
  };

  if (loadingProduct) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "60vh", fontFamily: "var(--font-heading)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr", gap: "1.5rem", marginBottom: "1.25rem" }}>
              <div>
                ...
                <input 
                  type="text" 
                  value={id} 
                  style={{ width: "100%", padding: "0.6rem", border: "1px solid #d1d5db", backgroundColor: "#f3f4f6", cursor: "not-allowed", textTransform: "uppercase" }}
                  readOnly 
                />
              </div>
              <div>
                ...
                <div key={size} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #f3f4f6", paddingBottom: "0.75rem" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                    <div className="qty-stepper">
                      ...
                      <input 
                        type="number" 
                        value={stock[size] !== undefined ? stock[size] : 0} 
          ...
          <div style={{ backgroundColor: "#fff", padding: "2.0rem 1.5rem", border: "1px solid #e5e7eb", borderRadius: "4px" }}>
            ...
            {images.length > 0 ? (
              ...
              <p style={{ fontSize: "0.75rem", color: "#6b7280", textAlign: "center", padding: "2rem 0", border: "2px dashed #e5e7eb" }}>
                No images uploaded. Select new files to replace.
              </p>
            )}
          </div>
